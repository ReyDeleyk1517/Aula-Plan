      ...
      if (filtroTipo != 'Todos') {
        ...
        switch (filtroTipo) {
          ...
          case 'Enlace': 
            ...
            break;
          case 'Otros':
            // 'Otros' si no cae en ninguna de las categorías anteriores
            const categoriasConocidas = [
              'pdf', 'doc', 'docx', 'txt', 'odt',
              'jpg', 'jpeg', 'png', 'webp', 'gif',
              'mp4', 'mov', 'avi', 'mkv', 'webm','enlace'
            ];
            coincideTipo = !categoriasConocidas.contains(ext) && ext != 'Enlace';
            break;
        }
      }
    ...
    try {
      ...
      for (var id in state.seleccionadosIds) {
        ...
        if (recurso.tipoArchivo == 'enlace') {
          ...
          archive.addFile(ArchiveFile("${recurso.nombre.replaceAll(' ', '_')}.txt", bytes.length, bytes));
        } else if (recurso.rutaArchivo != null) {
          final file = File(recurso.rutaArchivo!);
          if (await file.exists()) {
            final bytes = await file.readAsBytes();
            archive.addFile(ArchiveFile(recurso.rutaArchivo!.split('/').last, bytes.length, bytes));
          }
        }
      }
